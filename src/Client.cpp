...
//8)  - every client release p7_shared memory
...
P7_EXPORT void __cdecl P7_Exceptional_Flush()
{
    ...
    ////////////////////////////////////////////////////////////////////////////
    //trying to block p7_shared memory
...
////////////////////////////////////////////////////////////////////////////////
//Init_Crash_Handler()
//1) lock p7_shared memory (infinite)
//   - success - goto 5
//   - failed  - goto 2
//------------------------------------
//2) Fill sCrash structure
//3) Try to create&write p7_shared memory
//   - success - register Signal & exit
//   - failed  - goto 4
//------------------------------------
//4) lock p7_shared memory (infinite)
//   - success - goto 5
//   - failed  - exit, it is error
//------------------------------------
//5) Read p7_shared memory (without lock)
//6) Update p7_shared memory by new client
//7) increment reference counter
//8) write back p7_shared memory
//9) Unlock p7_shared memory
...
eClient_Status CClient::Init_Crash_Handler(tXCHAR **i_pArgs, tINT32 i_iCount)
{
    ...
    if (CShared::E_NOT_EXISTS == l_eLock) 
    {
        ...
        {
            //successful creation of p7_shared memory/////////////////////////////
            JOURNAL_DEBUG(m_pLog, TM("Register new crash handler"));

            l_bResult = TRUE;
            goto l_lblExit;
        }
        else //creation of p7_shared memory failed
        {
            //it wasn't possible to create p7_shared memory, possibly it was created
    ...
    {
        JOURNAL_ERROR(m_pLog, TM("Can't read crash p7_shared memory"));
        l_bResult = FALSE; 
        goto l_lblExit;
    }
    ...
    //no free room in p7_shared memory, exit !
    if (FALSE == l_bResult)
    {
    ...
    {
        JOURNAL_ERROR(m_pLog, TM("Can't write to crash p7_shared memory"));
        l_bResult = FALSE; 
        goto l_lblExit;
    }
...
//   - success - goto 2
//   - failed  - exit
//2) Read p7_shared memory
//3) find client and clear pointer
//4) decrease reference counter
//5) if reference counter = 0 delete p7_shared memory & unregister signal
//6) unlock p7_shared memory (if memory was deleted - it will just return error)
eClient_Status CClient::Uninit_Crash_Handler()
{
    ...
    {
        JOURNAL_ERROR(m_pLog, TM("Can't read from crash p7_shared memory"));
        l_bReturn = FALSE; 
        goto l_lblExit;
    }
